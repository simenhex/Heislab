...
func main(){
	...
	switch master{
		...
		listenSocket.SetDeadline(time.Now().Add(3*time.Second)) //This should be less
		
		//Listen on messages from master (while alive)
		for err == nil{
			count, err = reciveMessage(count, listenSocket)
			listenSocket.SetDeadline(time.Now().Add(1200*time.Millisecond)) //This should be less
		}

		//If this code run, the master is dead [long live the master]
		
		//Free up listenSocket
		listenSocket.Close()
